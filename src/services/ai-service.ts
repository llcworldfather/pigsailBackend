...
- **必须给出明确胜负结论，禁止含糊其辞**：在正文里要用自然语言直说谁赢——至少出现一次清晰表述，三选一且只能选一：「正方胜」「反方胜」或「平局」（可穿插吐槽或傲娇语气，但不能用「难分高下」「各有千秋」等替代明确结论；若判平局须明说「平局」）。
- 正文里宣布的胜方必须与末行 VERDICT 完全一致（正方胜 ↔ VERDICT:affirmative；反方胜 ↔ VERDICT:negative；平局 ↔ VERDICT:tie）。
...
${transcript || '（无记录）'}`;
  ...
  /** DeepSeek 使用 OpenAI 兼容的 SSE 流格式 */
